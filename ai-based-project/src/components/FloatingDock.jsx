...
const NAV_ITEMS = [
  ...
  { to: '/dsa-round',      icon: 'code',           label: 'DSA Round' },
  { to: '/interviewsetup', icon: 'video_chat',     label: 'Interviews' },
  { to: '/leaderboard',    icon: 'social_leaderboard', label: 'Rankings' },
  { to: '/pricing',        icon: 'workspace_premium', label: 'Upgrade' },
];

const DockNav = ({ position, isScrolling, setDockPosition }) => {
  const [hoveredIndex, setHoveredIndex] = useState(null);
  const isLeft = position === 'left';

  return (
    <motion.nav 
      initial={isLeft ? { x: -180, opacity: 0 } : { y: 180, opacity: 0 }}
      animate={{ x: 0, y: 0, opacity: 1, scale: 1 }}
      exit={isLeft ? { x: -180, opacity: 0 } : { y: 180, opacity: 0 }}
      transition={{ type: "spring", stiffness: 150, damping: 20 }}
      className={`fixed z-50 group border border-white/20 shadow-[0_8px_40px_rgba(0,0,0,0.6),0_0_0_1px_rgba(139,92,246,0.2)] flex items-center gap-4 ${
        isLeft 
          ? 'left-8 top-1/2 -translate-y-1/2 flex-col rounded-[2rem] py-6 px-3' 
          : 'bottom-8 left-1/2 -translate-x-1/2 flex-row rounded-full px-6 py-3'
...
const FloatingDock = () => {
  ...
  const isHiddenRoute = 
    ['/chat-interview', '/video-interview', '/voice-interview', '/interviewsetup', '/dsa-round'].includes(location.pathname) ||
