...
# 2. State
state_str = """  const [interviewStage, setInterviewStage] = useState("setup");
...
"""
content = content.replace('  const [interviewStage, setInterviewStage] = useState("setup");\n  const [feedback, setFeedback] = useState("");', state_str.strip("\n"))


# 3. UseEffects (replace the old stopStream cleanup)
old_cleanup = """  useEffect(() => {
    // Clean up when leaving page
    return () => stopStream();
  }, []);"""

new_effects = """  const toggleCamera = async () => {
    if (cameraEnabled) {
      stopStream();
...
content = content.replace(old_cleanup, new_effects)

# 4. GenerateQuestions and Persist feedback credentials and persona
content = content.replace(
...
# 5. Functions right before return
functions_before_return = """  const startInterviewCountdown = () => {
...
content = content.replace("  return (", functions_before_return, 1)

# 6. Replace the JSX block
start_idx = content.find('{interviewStage === "setup" && (')
end_idx = content.find('{interviewStage === "interview" && questions.length > 0 && (')
