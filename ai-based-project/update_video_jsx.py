...
# Add company functions before return
company_logic = """
  ...
  return (
"""
content = content.replace("  return (", company_logic)

# Replace the setup stage JSX
# We need to find the start of setup and end of setup.
...
if start_idx != -1 and end_idx != -1:
                    ...
                    <div className="space-y-1.5 relative">
                      ...
                      <AnimatePresence>
                        {showSuggestions && (
                          ...
                          >
                            {KNOWN_COMPANIES.filter(c => c.name.toLowerCase().includes(interviewDetails.company.toLowerCase())).map(c => (
                              <button
                                key={c.name}
    ...
    new_content = content[:start_idx] + setup_jsx + content[end_idx:]
    with open("src/Pages/VideoInterview.jsx", "w") as f:
        f.write(new_content)
    print("JSX updated successfully.")
else:
    print("Could not find start or end index for setup JSX.")
